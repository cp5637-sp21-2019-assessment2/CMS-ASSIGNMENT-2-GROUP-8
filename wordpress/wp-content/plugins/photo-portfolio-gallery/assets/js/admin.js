...
var portfoliosListNameChange = function (e) {
    document.getElementById("web_pace_portfolio_name").value = e.value;
};
jQuery(document).ready(function () {
	...
	jQuery('a.set-new-video').click(function (e) {
		e.preventDefault();
		var videoUrl = jQuery('#web_pace_edit_video_input').val();
		if (videoUrl == "") {
		...
		var videoEditNonce = jQuery(this).parents('#photo-portfolio-edit-video-wrapper').attr('data-see-video-nonce');
		var videoUrl = jQuery('input#web_pace_edit_video_input').val();
		var data = {
			videoEditNonce: videoEditNonce,
			videoUrl: videoUrl,
			post: 'see_new_video',
			action: 'photo_gallery_action'
		};
		jQuery.post(ajaxUrl, data, function (response) {
			response = JSON.parse(response);
			jQuery('#photo-portfolio-edit-video-wrapper .iframe-area').attr('src',response);
			jQuery('#photo-portfolio-edit-video-wrapper .text-area').html(videoUrl);
		});
	});
	jQuery('.web-pace-insert-edited-video-button').click(function (e) {
		e.preventDefault();
		var videoUrl = jQuery('#web_pace_edit_video_input').val();
		if (videoUrl == "") {
			alert("Please copy and past url from Youtube or Vimeo to insert into slider.");
		...
		var portfolioId = jQuery(this).parents('#photo-portfolio-edit-video-wrapper').attr('data-portfolio-id');
		var portfolioItemId = jQuery(this).parents('#photo-portfolio-edit-video-wrapper').attr('data-portfolio-item-id');
		var videoIndexInArray = jQuery(this).parents('#photo-portfolio-edit-video-wrapper').attr('data-video-index');
		var videoEditNonce = jQuery(this).parents('#photo-portfolio-edit-video-wrapper').attr('data-edit-video-nonce');
		var videoEditSafeLink = 'admin.php?page=web_pace_portfolio&task=portfolio_video_edit&portfolio_id='+portfolioId+'&id='+portfolioItemId+'&thumb='+videoIndexInArray+'&portfolio_video_edit_nonce='+videoEditNonce+'&TB_iframe=1&closepop=1';
	...
	jQuery('a.edit-video-button').on('click',function () {
		...
		var portfolioId = jQuery(this).parents('ul#images-list').attr('data-photo-portfolio-id');
		...
		jQuery('#photo-portfolio-edit-video-wrapper .iframe-area').attr('src',iframeSrc);
		jQuery('#photo-portfolio-edit-video-wrapper textarea.text-area').text(videoSrc);
		jQuery('#photo-portfolio-edit-video-wrapper').attr('data-video-index',videoIndexInArray);
		jQuery('#photo-portfolio-edit-video-wrapper').attr('data-portfolio-id',portfolioId);
		jQuery('#photo-portfolio-edit-video-wrapper').attr('data-portfolio-item-id',portfolioItemId);
		jQuery('#photo-portfolio-edit-video-wrapper').attr('data-edit-video-nonce',editThumbVideoNonce);
	});
	jQuery('.web-pace-insert-thumb-video-button').click(function (e) {
		e.preventDefault();
		var videoUrl = jQuery('#web_pace_add_video_input_thumb').val();
		if (videoUrl == "") {
			alert("Please copy and past url from Youtube or Vimeo to insert into slider.");
			return false;
		}
		...
		var addThumbVideoNonce = jQuery('#photo_gallery_add_videos_wrap').attr('data-add-thumb-video-nonce');
		var data = {
		...
		jQuery.post(ajaxUrl, data, function (response) {
			...
			var videoContainer = '<li class="editthisvideo editthisimage'+projectIndex+' ui-sortable-handle" data-video-index="'+videoIndex+'" data-iframe-src="'+iframeVideoSrc+'"> \
                ...
                <a class="thickbox edit-video-button" data-edit-thumb-video="" href="#TB_inline?width=700&amp;inlineId=photo-portfolio-edit-video&amp;width=753&amp;height=396"> \
	...
	jQuery('.web-pace-insert-video-button').click(function (e) {
		e.preventDefault();
		var videoUrl = jQuery('#web_pace_add_video_input').val();
		if (videoUrl == "") {
			alert("Please copy and past url from Youtube or Vimeo to insert into slider.");
			return false;
		}
		...
		var portfolioId = jQuery(this).parents('#photo_gallery_add_videos_wrap').attr('data-photo-portfolio-id');
		var portfolioVideoAddNonce = jQuery(this).parents('#photo_gallery_add_videos_wrap').attr('data-add-video-nonce');
		jQuery(this).parent().attr('action', 'admin.php?page=web_pace_portfolio&task=portfolio_video&id=' + portfolioId + '&portfolio_add_video_nonce=' + portfolioVideoAddNonce + '&closepop=1').submit();
	});
	jQuery('#web_pace_add_video_input').change(function () {
	...
	jQuery('.add-image-video .add-video-slide').on('click', function () {
		...
		jQuery('#photo_gallery_add_videos_wrap').attr('data-add-thumb-video-nonce',portfolioVideoAddNonce);
		jQuery('#photo_gallery_add_videos_wrap').attr('data-photo-portfolio-id', portfolioId);
	});
	jQuery('.button.add-video-slide').on('click', function () {
		jQuery('form.add-main-video').show();
		jQuery('form.add-thumb-video').hide();
		var portfolioId = jQuery(this).attr('data-photo-portfolio-id');
        var addVideoNonce = jQuery(this).attr('data-add-video-nonce');
		jQuery('#photo_gallery_add_videos_wrap').attr('data-photo-portfolio-id', portfolioId);
		jQuery('#photo_gallery_add_videos_wrap').attr('data-add-video-nonce', addVideoNonce);
	});
	...
	jQuery('#portfolio-view-tabs > li > a').click(function(){
		...
		jQuery('#adminForm').attr('action',"admin.php?page=Options_web_pace_portfolio_styles&task=save"+liID);
		return false;
	});
    ...
    var custom_uploader;
    jQuery('.web-pace-newuploader .button').click(function(e) {
        e.preventDefault();
        var button = jQuery(this);
        var id = button.attr('id').replace('_button', '');
