...
class Photo_Gallery_Portfolios {
	/**
	 * Load Portfolios admin page
	 */
	public function load_portfolio_page() {
		global $wpdb;
		$task = photo_gallery_get_portfolio_task();
		$id   = photo_gallery_get_portfolio_id();
		switch ( $task ) {
			case 'edit_cat':
				if ( $id ) {
			...
			case 'save':
				...
				break;
			case 'apply':
				$a = isset( $_REQUEST['webpace_portfolio_apply_portfolio_nonce'] );
				$b = wp_verify_nonce( $_REQUEST['webpace_portfolio_apply_portfolio_nonce'], 'apply_portfolio_' . $id );
				$c = wp_verify_nonce( $_REQUEST['webpace_portfolio_apply_portfolio_nonce'], 'remove_project_' . ( isset( $_GET['removeslide'] ) ? absint( $_GET['removeslide'] ) : '' ) );
	...
	public function show_portfolios_page() {
		...
		$cat_row_query = "SELECT id,name FROM " . $wpdb->prefix . "web_paceportfolio_portfolios WHERE sl_width=0";

		// get the total number of records
		$query = "SELECT COUNT(*) FROM " . $wpdb->prefix . "web_paceportfolio_portfolios" . $where;
...
(SELECT " . $wpdb->prefix . "web_paceportfolio_portfolios.name AS par_name," . $wpdb->prefix . "web_paceportfolio_portfolios.id FROM " . $wpdb->prefix . "web_paceportfolio_portfolios) AS g
 ON a.sl_width=g.id WHERE a.name LIKE '%" . $search_tag . "%'  group by a.id " . $order . " LIMIT " . $limit . ",20";
		...
		$rows      = photo_gallery_open_cat_in_tree( $rows );
		$query     = "SELECT  " . $wpdb->prefix . "web_paceportfolio_portfolios.ordering," . $wpdb->prefix . "web_paceportfolio_portfolios.id, COUNT( " . $wpdb->prefix . "web_paceportfolio_images.portfolio_id ) AS prod_count
FROM " . $wpdb->prefix . "web_paceportfolio_images, " . $wpdb->prefix . "web_paceportfolio_portfolios
WHERE " . $wpdb->prefix . "web_paceportfolio_images.portfolio_id = " . $wpdb->prefix . "web_paceportfolio_portfolios.id
GROUP BY " . $wpdb->prefix . "web_paceportfolio_images.portfolio_id ";
		...
		require_once( PHOTO_PORTFOLIO_TEMPLATES_PATH . DIRECTORY_SEPARATOR . 'admin' . DIRECTORY_SEPARATOR . 'photo-portfolio-admin-list.php' );
	}
	...
	public function edit_portfolio( $id ) {
		...
		if ( isset( $_GET["removeslide"] ) ) {
			if ( absint( $_GET['removeslide'] ) != $_GET['removeslide'] || !absint($_GET['removeslide'])) {
				wp_die( __('"removeslide" parameter is required to be not negative integer','photo-portfolio' ) );
			} else {
				$project_id = absint( $_GET["removeslide"] );
				$wpdb->query( "DELETE FROM " . $wpdb->prefix . "web_paceportfolio_images  WHERE id = " . $project_id );
			}
		}
		...
		$cat_row   = $wpdb->get_results( "SELECT * FROM " . $wpdb->prefix . "web_paceportfolio_portfolios WHERE id!=" . $id . " and sl_width=0" );
		$cat_row   = photo_gallery_open_cat_in_tree( $cat_row );
		$query     = $wpdb->prepare( "SELECT name,ordering FROM " . $wpdb->prefix . "web_paceportfolio_portfolios WHERE sl_width=%d  ORDER BY `ordering` ", $row->sl_width );
		$ord_elem  = $wpdb->get_results( $query );

		$query  = $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "web_paceportfolio_images where portfolio_id = %d order by ordering ASC  ", $row->id );
		$rowim  = $wpdb->get_results( $query );
		$query  = "SELECT * FROM " . $wpdb->prefix . "web_paceportfolio_portfolios order by id ASC";
		$rowsld = $wpdb->get_results( $query );

		require_once( PHOTO_PORTFOLIO_TEMPLATES_PATH . DIRECTORY_SEPARATOR . 'admin' . DIRECTORY_SEPARATOR . 'photo-portfolio-admin-images-list-view.php' );
	}
	...
	public function save_portfolio_data( $id ) {
		...
		update_option( 'photo_gallery_disable_right_click', sanitize_text_field( $_POST['disable_right_click'] ) );

		$query = $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "web_paceportfolio_portfolios WHERE id = %d", $id );
		$row   = $wpdb->get_row( $query );

		if ( isset( $_POST['photo_gallery_admin_image_hover_preview'] ) ) {
			$img_preview = sanitize_text_field( $_POST['photo_gallery_admin_image_hover_preview'] );
			update_option( 'photo_gallery_admin_image_hover_preview', $img_preview );
		} else {
			update_option( 'photo_gallery_admin_image_hover_preview', 'off' );
		}

		if ( isset( $_POST['changedvalues'] ) && $_POST['changedvalues'] != '' ) {
			$changedValues = preg_replace( '#[^0-9,]+#', '', $_POST['changedvalues'] );
			$query         = $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "web_paceportfolio_images where portfolio_id = %d AND id in (" . $changedValues . ") order by id ASC", $row->id );
			$rowim         = $wpdb->get_results( $query );
			foreach ( $rowim as $key => $rowimages ) {
				...
				$wpdb->update(
					$wpdb->prefix . "web_paceportfolio_images",
					array(
						'ordering'    => $orderBy,
		...
		if ( $_POST["imagess"] != '' ) {
			$query = $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "web_paceportfolio_images where portfolio_id = %d order by id ASC", $row->id );
			$rowim = $wpdb->get_results( $query );
			foreach ( $rowim as $key => $rowimages ) {
				$orderingplus = $rowimages->ordering + 1;
				$wpdb->query( $wpdb->prepare( "UPDATE " . $wpdb->prefix . "web_paceportfolio_images SET  ordering = %d  WHERE ID = %d ", $orderingplus, $rowimages->id ) );
			}
		...
		?>
		<div class="updated"><p><strong><?php _e( 'Item Saved', 'photo-portfolio' ); ?></strong></p></div>
		<?php
