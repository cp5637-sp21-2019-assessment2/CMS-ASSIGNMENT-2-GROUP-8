...
if ( ! function_exists( 'youtube_or_vimeo_portfolio' ) ) {
	 ...
	 * @return string
	 */
	function photo_gallery_youtube_or_vimeo_portfolio ($url ) {
	 ...
	 * @return array
	 */
	function photo_gallery_get_video_id_from_url( $url ) {
		if ( strpos( $url, 'youtube' ) !== false || strpos( $url, 'youtu' ) !== false ) {
			if ( preg_match( '%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $url, $match ) ) {
	 ...
	 * @return string
	 */
	function photo_gallery_get_image_from_video( $image_url ) {
		if ( strpos( $image_url, 'youtube' ) !== false || strpos( $image_url, 'youtu' ) !== false ) {
			$video_thumb     = photo_gallery_get_video_id_from_url( $image_url );
