...
global $wpdb;
$portfolio_wp_nonce = wp_create_nonce( 'web_pace_portfolio_nonce' );
$id                 = intval( $_GET['id'] );
?>
	...
	<div class="wrap">
		...
		<?php
		$path_site2  = PHOTO_PORTFOLIO_IMAGES_URL;
		$form_action = wp_nonce_url( 'admin.php?page=web_pace_portfolio&id=' . $id, 'apply_portfolio_' . $row->id, 'webpace_portfolio_apply_portfolio_nonce' );
		?>
						...
						foreach ( $rowsld as $rowsldires ) {
							if ( $rowsldires->id != $row->id ) {
								$edit_portfolio_safe_link = wp_nonce_url(
									'admin.php?page=web_pace_portfolio&task=edit_cat&id=' . $rowsldires->id,
									'edit_portfolio_' . $rowsldires->id,
									'webpace_portfolio_edit_portfolio_nonce'
								);
								?>
								<li>
									<a href="#"
									   onclick="window.location.href='<?php echo esc_url( 'admin.php?page=web_pace_portfolio&task=edit_cat&id=' . $rowsldires->id ) ?>'"><?php echo esc_html($rowsldires->name); ?></a>
								</li>
								<?php
							} else { ?>
								<li class="active"
								    onclick="this.firstElementChild.style.width = ((this.firstElementChild.value.length + 1) * 8) + 'px';"
								    style="background-image:url(<?php echo PHOTO_PORTFOLIO_IMAGES_URL . '/admin_images/edit.png'; ?>);cursor: pointer;">
					...
					<div id="post-body-content">
						...
						<div id="post-body">
							<div id="post-body-heading">
								<div id="img_preview">
									<h3><?php echo __( 'Projects / Images', 'photo-portfolio' ); ?></h3>
									<input type="hidden" name="imagess" id="_unique_name"/>
									<input type="hidden" name="photo_gallery_admin_image_hover_preview"
									       value="off"/>
									<label
										for="img_hover_preview"><?php _e( 'Image preview on hover', 'photo-portfolio' ); ?>
										<input type="checkbox" id="img_hover_preview"
										       name="photo_gallery_admin_image_hover_preview"
										       value="on" <?php if ( get_option( 'photo_gallery_admin_image_hover_preview' ) == 'on' )
											echo 'checked' ?>>
									</label>
								</div>
								<div class="web-pace-newuploader uploader  add-new-image">
								...
								<a href="#TB_inline?width=700&height=500&inlineId=photo_gallery_add_videos"
								   class="button button-primary add-video-slide thickbox" id="slideup3s"
								   value="iframepop" data-photo-portfolio-id="<?php echo $row->id; ?>"
								   data-add-video-nonce="<?php echo $add_video_nonce; ?>">
									<span class="wp-media-buttons-icon"></span>
									<?php echo __( 'Add Video', 'photo-portfolio' ); ?>
								</a>
							</div>
							<ul id="images-list" data-photo-portfolio-id="<?php echo $row->id; ?>">
								<?php
								$j      = 2;
											...
											$j ++; ?>' data-portfolio-item-id="<?php echo $rowimages->id; ?>">
												...
												<div class="image-container">
													<ul class="widget-images-list">
														...
														foreach ( $imgurl as $key1 => $img ) {
															...
															if ( photo_gallery_youtube_or_vimeo_portfolio( $img ) != 'image' ) {
																$video_id = photo_gallery_get_video_id_from_url( $img );
																if ( photo_gallery_youtube_or_vimeo_portfolio( $img ) == 'youtube' ) {
																...
																<li class="editthisvideo editthisimage<?php echo $key; ?><?php if ( $i == 0 ) {
																	...
																	<img class="editthisvideo"
																	     src="<?php echo photo_gallery_get_image_from_video( $img ); ?>"
																	     data-video-src="<?php echo esc_attr( $img ); ?>"
																	     alt="<?php echo esc_attr( $img ); ?>"/>
																	<div
																		class="play-icon <?php echo photo_gallery_youtube_or_vimeo_portfolio( $img ) == 'youtube' ? 'youtube-icon' : 'vimeo-icon'; ?>"></div>
																	<a class="thickbox edit-video-button"
																	   data-edit-thumb-video="<?php echo $edit_thumb_video_nonce; ?>"
																	   href="#TB_inline?width=700&height=500&inlineId=photo-portfolio-edit-video">
																...
																<li class="editthisimage editthisimage<?php echo $key; ?> <?php if ( $i == 0 ) {
																	...
																	<img
																		src="<?php echo esc_attr( photo_gallery_get_image_by_sizes_and_src( $img, array(), true ) ); ?>"
														...
														<li class="add-image-box">
															<div class="add-thumb-project">
																<img
																	src="<?php echo Photo_Gallery()->plugin_url() . '/assets/images/admin_images/plus.png'; ?>"
																	class="plus" alt=""/>
															</div>
															<div class="add-image-video">
																...
																<a title="Add video" class="add-video-slide thickbox"
																   ...
																   href="#TB_inline?width=700&height=500&inlineId=photo_gallery_add_videos">
																	<img
																		src="<?php echo Photo_Gallery()->plugin_url() . '/assets/images/admin_images/icon-video.png'; ?>"
																...
																<div class="add-image-slide" title="Add image">
																	<img
																		src="<?php echo Photo_Gallery()->plugin_url() . '/assets/images/admin_images/icon-img.png'; ?>"
												...
												<div class="image-options">
													<div class="options-container">
														<div>
															<input class="text_area" type="text"
															       placeholder="<?php echo __( 'Title', 'photo-portfolio' ); ?>"
                                                        ...
                                                        <textarea id="im_description<?php echo $rowimages->id; ?>"
                                                                  placeholder="<?php echo __( 'Description', 'photo-portfolio' ); ?>"
														...
														<div class="link-block">
															<input class="text_area url-input" type="text"
															       placeholder="<?php echo __( 'URL', 'photo-portfolio' ); ?>"
															...
															<label class="long"
																...
																<span><?php echo __( 'Open in new tab', 'photo-portfolio' ); ?></span>
																<input type="hidden"
													...
													<div class="category-container">
														<strong><?php echo __( 'Select Categories', 'photo-portfolio' ); ?></strong>
														<em>(<?php echo __( 'Press Ctrl And Select multiply', 'photo-portfolio' ); ?>
															)</em>
														<select id="multipleSelect" multiple="multiple" disabled>
															<?php
															$web_cat = explode( ",", $rowimages->category );
															foreach ( $myrows as $value ) {
																if ( ! empty( $value ) ) {
																	?>
																	<option <?php if ( in_array( str_replace( ' ', '_', $value ), str_replace( ' ', '_', $web_cat ) ) ) {
															...
															$remove_project_safe_link = wp_nonce_url(
																'admin.php?page=web_pace_portfolio&id=' . $row->id . '&task=apply&removeslide=' . $rowimages->id,
																'remove_project_' . $rowimages->id,
																'webpace_portfolio_apply_portfolio_nonce'
															);
															?>
														</select>
													...
													<div class="remove-image-container">
														<a class="button remove-image"
														   href="<?php echo $remove_project_safe_link; ?>"><?php echo __( 'Remove Project', 'photo-portfolio' ); ?></a>
													</div>
												</div>
												<div class="clear"></div>
											</li>
														...
														foreach ( $imgurl as $key1 => $img ) {
															...
															if ( photo_gallery_youtube_or_vimeo_portfolio( $img ) != 'image' ) :
																...
																<li class="editthisvideo editthisimage<?php echo $key; ?><?php if ( $i == 0 ) {
																    ...
																    data-iframe-src="<?php echo $iframe_video_src; ?>">
																	<img class="editthisvideo"
																	     src="<?php echo esc_attr( photo_gallery_get_image_from_video( $img ) ); ?>"
																	     data-video-src="<?php echo esc_attr( $img ); ?>"
																	     alt="<?php echo esc_attr( $img ); ?>"/>
																	<div
																		class="play-icon <?php echo photo_gallery_youtube_or_vimeo_portfolio( $img ) == 'youtube' ? 'youtube-icon' : 'vimeo-icon'; ?>"></div>
														...
														<li class="add-image-box">
															...
															<div class="add-image-video">
																...
																<a title="Add video" class="add-video-slide thickbox"
																   ...
																   href="#TB_inline?width=700&height=500&inlineId=photo_gallery_add_videos">
																	<img
																		src="<?php echo esc_attr( Photo_Gallery()->plugin_url() . '/assets/images/admin_images/icon-video.png' ); ?>"
																		title="Add video" alt="" class="plus"/>
																	<input type="button"
																...
																<div class="add-image-slide" title="Add image">
																	<img
																		src="<?php echo esc_attr( Photo_Gallery()->plugin_url() . '/assets/images/admin_images/icon-img.png' ); ?>"
																		title="Add image" alt="" class="plus"/>
																	<input type="button"
																	       class="button<?php echo $rowimages->id; ?> wp-media-buttons-icon add-image"
																	       id="unique_name_button<?php echo $rowimages->id; ?>"
					...
					<div id="postbox-container-1" class="postbox-container">
						<div id="side-sortables" class="meta-box-sortables ui-sortable">
							<div id="portfolio-unique-options" class="postbox">
								<h3 class="hndle">
									<span><?php echo __( 'Select The Portfolio/Gallery View', 'photo-portfolio' ); ?></span>
								</h3>
								<ul id="portfolio-unique-options-list">
									<li>
										<label
											for="web_pace_portfolio_name"><?php echo __( 'Portfolio Name', 'photo-portfolio' ); ?></label>
										<input type="text" name="name" id="web_pace_portfolio_name"
										       value="<?php echo esc_html( stripslashes( $row->name ) ); ?>"
										       onkeyup="sidebarNameChange(this)">
									</li>
									<li style="display:none;">
										<label
											for="sl_width"><?php echo __( 'The requested action is not valid.', 'photo-portfolio' ); ?></label>
										<input type="text" name="sl_width" id="sl_width" value="1111"
										       class="text_area"/>
									</li>
									<li style="display:none;">
										<label
											for="sl_height"><?php echo __( 'Height', 'photo-portfolio' ); ?></label>
									...
									<li style="display:none;">
										<label
											for="pause_on_hover"><?php echo __( 'Pause on hover', 'photo-portfolio' ); ?></label>
									...
									<li>
										<label
											for="portfolio_effects_list"><?php echo __( 'Select The View', 'photo-portfolio' ); ?></label>
									...
									<li style="display:none;" class="for-content-slider">
										<label
											for="sl_pausetime"><?php echo __( 'Pause time', 'photo-portfolio' ); ?></label>
									...
									<li style="display:none;" class="for-content-slider">
										<label
											for="sl_changespeed"><?php echo __( 'Change speed', 'photo-portfolio' ); ?></label>
									...
									<li class="no-content-slider no-full-width">
										<label
											for="slider_effect"><?php echo __( 'Show In Center', 'photo-portfolio' ); ?></label>
										...
										<a class="portfolio-pro-link probuttonlink"
										   href="http://webpace.net/wordpress-photo-portfolio-gallery-plugin/"
									...
									<li style="display:none;margin-top:10px" class="for-content-slider">
										<label
											for="autoslide"><?php echo __( 'Autoslide ', 'photo-portfolio' ); ?></label>
									...
									<li>
										<label
											for="disable_right_click"><?php echo __( 'Disable Image Right Click', 'photo-portfolio' ); ?></label>
										<select name="disable_right_click" id="disable_right_click">
											<option <?php if ( get_option( 'photo_gallery_disable_right_click' ) == 'off' ) {
												echo 'selected';
											} ?> value="off">Off
											</option>
											<option <?php if ( get_option( 'photo_gallery_disable_right_click' ) == 'on' ) {
												echo 'selected';
											} ?> value="on">On
											</option>
								...
								<div class="inside2">
									<ul>
										<li class="allowIsotope">
											<?php echo __( ' Show Sorting Buttons', 'photo-portfolio' ); ?> :
										...
										<li class="allowIsotope">
											<?php echo __( ' Show Category Buttons', 'photo-portfolio' ); ?> :
							...
							<div class="postbox">
								<h3 class="hndle"><span><?php echo __( 'Categories', 'photo-portfolio' ); ?>:</span>&nbsp;&nbsp;
									<a class="portfolio-pro-link probuttonlink"
									   href="http://webpace.net/wordpress-photo-portfolio-gallery-plugin/"
								...
								<div class="inside">
									<ul>
												...
												if ( ! empty( $value ) ) : ?>
													<li class="hndle">
														...
														<span id="delete_cat" style="" value="a"><img
																src="<?php echo PHOTO_PORTFOLIO_IMAGES_URL . "/admin_images/delete1.png"; ?>"
																width="9" height="9" value="a"></span>
														<span id="edit_cat" style=""><img
																src='<?php echo PHOTO_PORTFOLIO_IMAGES_URL . "/admin_images/edit3.png"; ?>'
										...
										<li id="add_cat_input" style="">
											...
											<a style=""
											   id="add_new_cat_buddon">+<?php echo __( 'Add New Category', 'photo-portfolio' ); ?></a>
										</li>
									</ul>
							...
							<div class="postbox">
								<h3 class="hndle"><span><?php echo __( 'Loading Icons', 'photo-portfolio' ); ?></span>
								</h3>
								<div class="inside">
									<ul id="portfolio-unique-options-list" class="for_loading">
										<li>
											<label><?php echo __( ' Show Loading Icon', 'photo-portfolio' ); ?>
												:</label>
										...
										<li class="loading_opton">
											<label for="portfolio_load_icon"
											       style="width: 100%"><?php echo __( 'Image while portfolio loads:', 'photo-portfolio' ); ?></label>
							...
							<div id="portfolio-shortcode-box" class="postbox shortcode ms-toggle">
								<h3 class="hndle"><span><?php echo __( 'Usage', 'photo-portfolio' ); ?></span></h3>
								<div class="inside">
									<ul>
										<li rel="tab-1" class="selected">
											<h4><?php echo __( 'Shortcode', 'photo-portfolio' ); ?></h4>
											<p><?php echo __( 'Copy &amp; paste the shortcode directly into any WordPress post or page', 'photo-portfolio' ); ?>
												.</p>
											<textarea class="full"
											          readonly="readonly">[web_pace_portfolio id="<?php echo $row->id; ?>"]</textarea>
										</li>
										<li rel="tab-2">
											<h4><?php echo __( 'Template Include', 'photo-portfolio' ); ?></h4>
											<p><?php echo __( 'Copy &amp; paste this code into a template file to include the slideshow within your theme', 'photo-portfolio' ); ?>
												.</p>
											<textarea class="full" readonly="readonly">&lt;?php echo do_shortcode("[web_pace_portfolio id='<?php echo $row->id; ?>']"); ?&gt;</textarea>
										</li>
									</ul>
								</div>
							</div>
...
<?php
require_once( PHOTO_PORTFOLIO_TEMPLATES_PATH . DIRECTORY_SEPARATOR . 'admin' . DIRECTORY_SEPARATOR . 'photo-portfolio-admin-video-add-view.php' );
require_once( PHOTO_PORTFOLIO_TEMPLATES_PATH . DIRECTORY_SEPARATOR . 'admin' . DIRECTORY_SEPARATOR . 'photo-portfolio-admin-video-edit-view.php' );
?>