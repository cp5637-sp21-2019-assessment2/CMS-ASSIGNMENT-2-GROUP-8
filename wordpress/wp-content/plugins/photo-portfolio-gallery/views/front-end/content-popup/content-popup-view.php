...
<section id="web_pace_portfolio_content_<?php echo esc_attr($portfolioID); ?>"
         ...
         data-image-behaviour="<?php echo esc_attr($photo_gallery_get_options['photo_gallery_port_natural_size_contentpopup']); ?>">
    ...
    <div id="web_pace_portfolio_options_and_filters_<?php echo esc_attr($portfolioID); ?>">
        ...
        <?php if ($portfolioShowSorting == "on") { ?>
            <div id="web_pace_portfolio_options_<?php echo esc_attr($portfolioID); ?>"
                 data-sorting-position="<?php echo esc_attr($photo_gallery_get_options["photo_gallery_ht_view2_sorting_float"]); ?>">
                <ul class="sort-by-button-group clearfix">
                    <?php if ($photo_gallery_get_options["photo_gallery_ht_view2_sorting_name_by_default"] != ''): ?>
                        <li><a href="#sortBy=original-order" data-option-value="original-order" class="selected"
                               data><?php echo esc_attr($photo_gallery_get_options["photo_gallery_ht_view2_sorting_name_by_default"]); ?></a>
                        </li>
                    <?php endif; ?>
                    <?php if ($photo_gallery_get_options["photo_gallery_ht_view2_sorting_name_by_id"] != ''): ?>
                        <li><a href="#sortBy=load_date"
                               data-option-value="load_date"><?php echo esc_attr($photo_gallery_get_options["photo_gallery_ht_view2_sorting_name_by_id"]); ?></a>
                        </li>
                    <?php endif; ?>
                    <?php if ($photo_gallery_get_options["photo_gallery_ht_view2_sorting_name_by_name"] != ''): ?>
                        <li><a href="#sortBy=name"
                               data-option-value="name"><?php echo esc_attr($photo_gallery_get_options["photo_gallery_ht_view2_sorting_name_by_name"]); ?></a>
                        </li>
                    <?php endif; ?>
                    <?php if ($photo_gallery_get_options["photo_gallery_ht_view2_sorting_name_by_random"] != ''): ?>
                        <li id="random"><a data-option-value="random"
                                           href='#random'><?php echo esc_attr($photo_gallery_get_options["photo_gallery_ht_view2_sorting_name_by_random"]); ?></a>
                        </li>
                    <?php endif; ?>
                </ul>
                <ul id="port-sort-direction" class="option-set clearfix">
                    <?php if ($photo_gallery_get_options["photo_gallery_ht_view2_sorting_name_by_asc"] != ''): ?>
                        <li><a href="#sortAscending=true" data-option-value="true" data-option-key="number"
                               class="selected"><?php echo esc_attr($photo_gallery_get_options["photo_gallery_ht_view2_sorting_name_by_asc"]); ?></a>
                        </li>
                    <?php endif; ?>
                    <?php if ($photo_gallery_get_options["photo_gallery_ht_view2_sorting_name_by_desc"] != ''): ?>
                        <li><a href="#sortAscending=false" data-option-key="number"
                               data-option-value="false"><?php echo esc_attr($photo_gallery_get_options["photo_gallery_ht_view2_sorting_name_by_desc"]); ?></a>
                        </li>
                    <?php endif; ?>
                </ul>
            </div>
        ...
        if ($portfolioShowFiltering == "on") { ?>
            <div id="web_pace_portfolio_filters_<?php echo esc_attr($portfolioID); ?>"
                 data-filtering-position="<?php echo esc_attr($photo_gallery_get_options["photo_gallery_ht_view2_filtering_float"]); ?>">
                <ul>
                    <li rel="*">
                        <a><?php echo esc_attr($photo_gallery_get_options["photo_gallery_ht_view2_cat_all"]); ?></a>
                    </li>
                    <?php
...
<?php } ?>
    <div id="web_pace_portfolio_container_<?php echo esc_attr($portfolioID); ?>"
         class="web_pace_portfolio_container super-list variable-sizes clearfix view-<?php echo esc_attr($view_slug); ?>" <?php if ($sortingFloatPopup == "top" && $filteringFloatPopup == "top") {
        ...
        foreach ($images as $key => $row) {
            ...
            ?>
            <div id="web_pace_portfolio_pupup_element_<?php echo esc_attr($row->id); ?>_child"
                        ...
                        switch (photo_gallery_youtube_or_vimeo_portfolio($imgurl[0])) {
                            case 'image': ?>
                                <img alt="<?php echo esc_attr($row->name); ?>"
                                     ...
                                     src="<?php if ($photo_gallery_get_options['photo_gallery_port_natural_size_contentpopup'] == 'resize') {
                                         echo esc_url(photo_gallery_get_image_by_sizes_and_src($imgurl[0], array(
                                             $photo_gallery_get_options['photo_gallery_ht_view2_element_width'],
                                             $photo_gallery_get_options['photo_gallery_ht_view2_element_height']
                                         ), false));
                                     } else {
                ...
                <?php if ($row->name != '' || $row->sl_url != ''): ?>
                    <div class="title-block_<?php echo $portfolioID; ?>">
                        ...
                        <?php if ($photo_gallery_get_options["photo_gallery_ht_view2_element_show_linkbutton"] == 'on' && $link != '') { ?>
                            <div class="button-block"><a
                                        href="<?php echo esc_url($row->sl_url); ?>" <?php if ($row->link_target == "on") {
                                    echo 'target="_blank"';
                                } ?> ><?php echo $photo_gallery_get_options["photo_gallery_ht_view2_element_linkbutton_text"]; ?></a>
                            </div>
                        <?php } ?>
                    </div>
                <?php endif; ?>
...
<ul id="web_pace_portfolio_popup_list_<?php echo esc_attr($portfolioID); ?>" class="web_pace_portfolio_popup_list">
    <?php
    $changePopup = 1;
    foreach ($images as $key => $row) {
        ...
        <li class="pupup-element <?php foreach ($catForFilter as $catForFilterValue) {
            echo str_replace(" ", "_", $catForFilterValue) . " ";
        } ?>" id="web_pace_portfolio_pupup_element_<?php echo esc_attr($row->id); ?>">
            ...
            <div class="popup-wrapper popup-wrapper_<?php echo esc_attr($portfolioID); ?>">
                ...
                <div class="right-block">
                    <?php if ($photo_gallery_get_options["photo_gallery_ht_view2_show_popup_title"] == 'on') { ?>
                        <h3
                                class="title"><?php echo $row->name; ?></h3><?php } ?>

                    <?php if ($photo_gallery_get_options["photo_gallery_ht_view2_thumbs_position"] == 'before' and $photo_gallery_get_options["photo_gallery_ht_view2_show_thumbs"] == 'on' && count($imgurl) != 1) { ?>
                        <div>
                                            ...
                                            case 'image':
                                                ...
                                                <a href="<?php echo esc_url($img); ?>"
                                                   ...
                                                   title="<?php echo esc_attr($row->name); ?>"><img
                                                            src="<?php echo esc_url(photo_gallery_get_image_by_sizes_and_src($img, array(
                                                                $photo_gallery_get_options['photo_gallery_ht_view2_thumbs_width'],
                                                                $photo_gallery_get_options['photo_gallery_ht_view2_thumbs_height']
                                                            ), true)); ?>"></a>
                    ...
                    <?php if ($photo_gallery_get_options["photo_gallery_ht_view2_show_description"] == 'on') { ?>
                        <div class="description"><?php echo $row->description; ?></div><?php } ?>

                    <?php if ($photo_gallery_get_options["photo_gallery_ht_view2_thumbs_position"] == 'after' and $photo_gallery_get_options["photo_gallery_ht_view2_show_thumbs"] == 'on' && count($imgurl) != 1) { ?>
                        <div>
                    ...
                    <?php if ($photo_gallery_get_options["photo_gallery_ht_view2_show_popup_linkbutton"] == 'on' && $link != '') { ?>
                        <div class="button-block">
                            <a href="<?php echo esc_url($link); ?>" <?php if ($row->link_target == "on") {
                                echo 'target="_blank"';
                            } ?>><?php echo esc_attr($photo_gallery_get_options["photo_gallery_ht_view2_popup_linkbutton_text"]); ?></a>
                        </div>
                    <?php } ?>
                    <div style="clear:both;"></div>
                </div>
