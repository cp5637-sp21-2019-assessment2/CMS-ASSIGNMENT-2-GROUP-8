...
<div id="p-main-slider_<?php echo esc_attr($portfolioID); ?>"
     class="photo-portfolio-content p-main-slider liquid-slider view-<?php echo $view_slug; ?>"
		...
		<div class="slider-content">
			<div class="slider-content-wrapper slide_number<?php echo esc_attr($group_key1); ?>">
				<div class="image-block_<?php echo esc_attr($portfolioID); ?>">
						...
						switch ( photo_gallery_youtube_or_vimeo_portfolio( $imgurl[0] ) ) {
								...
								<a class="portfolio-group<?php echo esc_attr($group_key1); ?> portfolio-group-slider_<?php echo esc_attr($portfolioID); ?>_<?php echo esc_attr($group_key1); ?>"
								   ...
								   title="<?php echo esc_attr( $row->name ); ?>" data-groupID="<?php echo esc_attr($group_key1);?>"><img
										...
										src="<?php echo esc_url( photo_gallery_get_image_by_sizes_and_src( $imgurl[0], array(
											$photo_gallery_get_options['photo_gallery_ht_view5_main_image_width'],
											''
										), false ) ); ?>"/></a>
								<?php
								break;
							case 'youtube':
								...
								<a href="https://www.youtube.com/embed/<?php echo $videourl[0]; ?>"
								   ...
								   class="portfolio-group<?php echo esc_attr($group_key1); ?> web_pace_portfolio_item pyoutube portfolio-group-slider_<?php echo esc_attr($portfolioID); ?>_<?php echo esc_attr($group_key1); ?> add-H-relative add-H-block"
							...
							case 'vimeo':
								...
								?>
								<a class="portfolio-group<?php echo esc_attr($group_key1); ?> web_pace_portfolio_item pvimeo portfolio-group-slider_<?php echo esc_attr($portfolioID); ?>_<?php echo esc_attr($group_key1); ?>   add-H-relative add-H-block"
					...
					<?php if ( $photo_gallery_get_options["photo_gallery_ht_view5_show_thumbs"] ) { ?>
						<div>
								...
								foreach ( $imgurl as $key => $img ) {
									switch ( photo_gallery_youtube_or_vimeo_portfolio( $img ) ) {
                                                   ...
                                                   data-groupID="<?php echo esc_attr($group_key1);?>"><img alt="<?php echo esc_attr( $row->name ); ?>"
                                                    data-title=" <?php echo photo_gallery_get_image_title($img); ?>"
														src="<?php echo esc_url( photo_gallery_get_image_by_sizes_and_src( $img, array(
															$photo_gallery_get_options['photo_gallery_ht_view5_thumbs_width'],
															$photo_gallery_get_options['photo_gallery_ht_view5_thumbs_height']
														), true ) ); ?>"></a>
											</li>
											<?php
											break;
										case 'youtube':
											...
											<li>
												<a href="https://www.youtube.com/embed/<?php echo esc_attr($videourl[0]); ?>"
                                                   data-description=" <?php echo esc_attr( $row->description ); ?>"
												   class="portfolio-group<?php echo $group_key1; ?> web_pace_portfolio_item pyoutube portfolio-group-slider_<?php echo esc_attr($portfolioID); ?>_<?php echo esc_attr($group_key1); ?>  add-H-relative" data-groupID="<?php echo esc_attr($group_key1);?>">
										...
										case 'vimeo':
											...
											$imgsrc = $hash[0]['thumbnail_large']; ?>
											<li>
												<a class="portfolio-group<?php echo esc_attr($group_key1); ?> web_pace_portfolio_item pvimeo portfolio-group-slider_<?php echo esc_attr($portfolioID); ?>_<?php echo esc_attr($group_key1); ?>  add-H-relative"
												   href="http://player.vimeo.com/video/<?php echo esc_attr($videourl[0]); ?> "
                                                   data-description=" <?php echo esc_attr( $row->description ); ?>"
				...
				<div class="right-block">
					...
					<?php if ( $photo_gallery_get_options["photo_gallery_ht_view5_show_description"] == 'on' ) { ?>
						<div class="description"><?php echo $row->description; ?></div><?php } ?>
					<?php if ( $photo_gallery_get_options["photo_gallery_ht_view5_show_linkbutton"] == 'on' && $photo_gallery_get_options["photo_gallery_ht_view5_linkbutton_text"] != '' && $link != '' ) { ?>
						<div class="button-block">
							<a class=""
							   href="<?php echo esc_url( $link ); ?>" <?php if ( $row->link_target == "on" ) {
								echo 'target="_blank"';
							} ?>><?php echo esc_attr($photo_gallery_get_options["photo_gallery_ht_view5_linkbutton_text"]); ?></a>
						</div>
					<?php } ?>
				</div>
			</div>
